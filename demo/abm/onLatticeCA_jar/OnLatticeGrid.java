...
public class OnLatticeGrid extends AgentGrid2D<Cell> implements SerializableModel {
     ...
     * --- Parameters ----
     * ======================================================================== */
    int xDim = 200; // x-dimension of domain (in lattice sites)
    int yDim = 200; // y-dimension of domain (in lattice sites)
    ...
    double drugEffect_div_S = .3; // Relative amount by which drug reduces division rate of sensitive cells in [0,1].
    double drugEffect_div_R = 0.; // Relative amount by which drug reduces division rate of resistant cells in [0,1].
    int[] hood = Util.MooreHood(false); // Define division in von Neumann neighbourhood. Argument indicates that we don't want the center position returned (can only divide into neighbouring squares).
    ...
    public void StepCells() {
        ...
        for (Cell c : this) { //iterate over all cells in the grid
            ...
            divRate = c.divisionRate*(1-c.drugEffect_div*currDrugConcentration);
            totPropensity = (divRate + c.movementRate +
                    c.deathRate) * dt;
            ...
            if (rn.Double() < totPropensity) {
                r = rn.Double();
                // 1. Division
                if (r < ((divRate * dt) / totPropensity)) {
                    if (c.HasSpace()) {
                        c.Divide();
                        cellCountsArr[c.resistance] += 1;
                    }
                // 2. Movement
                } else if (r < ((divRate + c.movementRate) * dt) / totPropensity) {
                    currPos = c.Isq();
                    moveSuccessB = c.Move();
                    if (moveSuccessB) {
